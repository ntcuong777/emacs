...
- Use `nix-shell` with the `shell.nix` file at project root to build, e.g., `nix-shell --run 'make -j8' shell.nix`
...
- Fresh repo workflow is `nix-shell shell.nix -> ./autogen.sh -> ./configure -> make` (`INSTALL.REPO`).
