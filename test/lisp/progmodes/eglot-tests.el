...
(ert-deftest eglot-test-fsevents-recursive-watch ()
  "Use one recursive FSEvents watch without enumerating project directories."
  (let ((file-notify--library 'fsevents)
        (server 'eglot-test-server)
        (project 'eglot-test-project)
        (watch-calls nil)
        (project-files-called nil)
        (file-watches (make-hash-table :test #'equal)))
    (ert-with-temp-directory root
      (let ((child (expand-file-name "child" root)))
        (make-directory child)
        (cl-letf* (((symbol-function 'eglot--project)
                    (lambda (_server) project))
                   ((symbol-function 'project-root)
                    (lambda (_project) root))
                   ((symbol-function 'eglot--count-file-watches)
                    (lambda () 0))
                   ((symbol-function 'eglot--file-watches)
                    (lambda (_server) file-watches))
                   ((symbol-function 'project-files)
                    (lambda (&rest _args)
                      (setq project-files-called t)
                      nil))
                   ((symbol-function 'file-notify-add-watch)
                    (lambda (file flags callback)
                      (push (list file flags callback) watch-calls)
                      'test-watch))
                   ((symbol-function 'eglot--find-buffer-visiting)
                    (lambda (&rest _args) nil))
                   ((symbol-function 'jsonrpc-notify)
                    (lambda (&rest _args) nil)))
          (eglot--watch-globs
           server 'test-registration
           (list (cons (lambda (_file) t) 1))
           nil t)
          (should (= 1 (length watch-calls)))
          (pcase-let ((`(,file ,flags ,callback) (car watch-calls)))
            (should (equal root file))
            (should (equal '(change recursive) flags))
            ;; A created directory is already covered by the recursive watch.
            (funcall callback (list 'test-watch 'created child)))
          (should (= 1 (length watch-calls)))
          (should-not project-files-called))))))
